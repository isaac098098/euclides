#!/bin/bash

cp -r ~/.config/alacritty ~/euclides/.config
cp -r ~/.config/awesome ~/euclides/.config
cp -r ~/.config/dunst ~/euclides/.config
cp -r ~/.config/gtk-3.0/settings.ini ~/euclides/.config/gtk-3.0/
cp -r ~/.config/i3 ~/euclides/.config
cp -r ~/.config/nvim ~/euclides/.config
cp -r ~/.config/picom ~/euclides/.config
cp ~/.config/rofi/base16-tomorrow-night.rasi ~/euclides/.config/rofi
cp ~/.config/rofi/config.rasi ~/euclides/.config/rofi
cp ~/.config/rofi/scripts/edit_notes.sh ~/euclides/.config/rofi/scripts
cp ~/.config/rofi/scripts/compile_notes.sh ~/euclides/.config/rofi/scripts
cp ~/.config/rofi/scripts/select_course.sh ~/euclides/.config/rofi/scripts
cp ~/.config/rofi/scripts/select_books.sh ~/euclides/.config/rofi/scripts
cp ~/.config/rofi/scripts/edit_card.sh ~/euclides/.config/rofi/scripts
cp ~/.config/rofi/scripts/delete_card.sh ~/euclides/.config/rofi/scripts
cp ~/.config/rofi/scripts/swap_cards.sh ~/euclides/.config/rofi/scripts
cp ~/.config/rofi/scripts/create_card.sh ~/euclides/.config/rofi/scripts
cp -r ~/.config/sxhkd ~/euclides/.config
cp -r ~/.config/zathura ~/euclides/.config
cp -r ~/.config/fontconfig ~/euclides/.config
cp -r ~/.config/inkscape/templates/default.svg ~/euclides/.config/inkscape/templates

cp -r ~/.Xresources ~/euclides/home
cp -r ~/.bash_profile ~/euclides/home
cp -r ~/.bashrc ~/euclides/home
cp -r ~/.vimrc ~/euclides/home
cp -r ~/.xinitrc ~/euclides/home
cp -r ~/.mpv/config ~/euclides/home
